...
public class Chunk : MonoBehaviour
{
    ...
    public void GetInformation(GameObject SpikeP, GameObject MeatBallP, GameObject CrystalP, GameObject Snake, SpawnController Folder, float Chance, int Count, Material Color, Material SnakeColor, List<Material> SnakeColors)
    {
        SpikePrefab = SpikeP;
    ...
    void Create()
    {
        ...
        if (Count > 0)
        {
            ...
            for (int i = -72; i < 72; i += 2)
            {
                ...
                if (Random.Range(0 - Count * 0.01f, 100) > CurChance - StaticOptions.Difficulty && SkipSpike < i)
                {
                    CreateSpike(i);
                }
    ...
    void CreateSpike(int i)
    {
        ...
        for (int a = -6; a <= 6; a++)
        {
            ...
            if (Random.Range(0, 100) > CurChance && ObjectCount < 2)
            {
                ...
                Dangers[Dangers.Count - 1].transform.localPosition = new Vector3(Count * 150 + i, 1 + StaticOptions.Difficulty / 2, a);
